...
const ReliefPointMapLeaflet: React.FC<ReliefPointMapLeafletProps> = ({
  ...
  useEffect(() => {
    const fetchPoints = async () => {
      try {
        const url = stormId
          ? `${API_BASE}/relief-point?stormId=${stormId}`
          : `${API_BASE}/relief-point`;
  ...
  const filteredPoints = points
    .filter((p) => {
      const matchesType =
        (showNeeds && p.type === "need") ||
        (showSupplies && p.type === "supply");
  ...
  const handleSubmitReport = async () => {
    try {
      ...
      const res = await fetch(`${API_BASE}/relief-point`, {
        method: "POST",
      ...
      <Box sx={{ position: "relative", height: "400px", width: "100%" }}>
        ...
        {selectedPoint && (
          ...
          >
            ...
            <Typography
              variant="caption"
              color={selectedPoint.verified ? "green" : "gray"}
            >
