...
const FitBounds: React.FC<{ points: ReliefPoint[], stormCenter: { lat: number; lng: number } | null }> = ({ points, stormCenter }) => {
    const map = useMap();
    useEffect(() => {
        let bounds: L.LatLngBounds | undefined;
        const validPoints = points.filter(p => p.location?.coordinates?.length === 2).map(p => [p.location.coordinates[1], p.location.coordinates[0]] as [number, number]);
...
const ReliefPointManager: React.FC = () => {
    ...
    const handleOpenModal = (type: 'supply' | 'need') => {
        ...
        setFormData({
            ...
            description: '',
            location: { type: 'Point', coordinates: [0, 0] },
            surplus: type === 'supply' ? [{ type: 'thực phẩm', quantity: 0, note: '' }] : [],
            needs: type === 'need' ? [{ type: 'người mắc kẹt', quantity: 0, note: '' }] : [],
    ...
    const handleLocationSelect = (lat: number, lng: number) => {
        setFormData({ ...formData, location: { type: 'Point', coordinates: [lng, lat] } }); // Lưu [lng, lat]
    };
        ...
        <Box p={3}>
            ...
            <Stack direction="row" spacing={2} alignItems="center" mb={3}>
                ...
                {selectedStorm && (() => {
                    ...
                    return (
                        <>
                            <Chip
                                label={storm.isActive ? '🟢 Đang hoạt động' : '⚪ Đã kết thúc'}
                            ...
                            {storm.isActive ? (
                                <Button
                                    ...
                                    color="error"
                                    onClick={async () => {
                                        if (window.confirm("Bạn có chắc chắn muốn kết thúc bão này không?")) {
                                            try {
            ...
            {selectedStorm && (
                <Stack direction="row" spacing={2} alignItems="center" mb={3}>
                    ...
                    <Select
                        value={filterType}
                        onChange={(e) => setFilterType(e.target.value as 'all' | 'need' | 'supply')}
                        size="small"
                        sx={{ minWidth: 150 }}
                    >
            ...
            <Dialog open={openModal} onClose={handleCloseModal} maxWidth="md" fullWidth>
                <DialogTitle>{modalType === 'supply' ? 'Tạo điểm cung cấp' : 'Tạo điểm cần giúp đỡ'}</DialogTitle>
                <DialogContent>
                    <TextField
                        label="Tên điểm"
                        name="name"
                    ...
                    {/* Bản đồ chọn vị trí */}
                    <Typography variant="body1" mb={1}>Chọn vị trí trên bản đồ (click để chọn):</Typography>
                    <Box sx={{ height: '300px', width: '100%', mb: 2 }}>
                        <MapContainer center={[18.333, 105.9]} zoom={8} style={{ height: '100%', width: '100%' }}>
                            <TileLayer
                                url="https://{s}.tile.openstreetmap.org/{z}/{x}/{y}.png"
                                attribution='© <a href="https://www.openstreetmap.org/copyright">OpenStreetMap</a> contributors'
                            />
                    ...
                    <Typography variant="body2" color="textSecondary" mb={2}>
                        Chọn vị trí trên bản đồ (click để chọn) (kinh độ - vĩ độ): {formData.location?.coordinates[0] ?? 0} - {formData.location?.coordinates[1] ?? 0}
                    </Typography>
                    ...
                    {modalType === 'supply' && (
                        <>
                            <Typography variant="subtitle1" mt={2}>Dư thừa:</Typography>
                            {formData.surplus.map((item, index) => (
                                <Stack key={index} direction="row" spacing={1} alignItems="center" mb={1}>
                                    <FormControl fullWidth margin="normal">
                                        ...
                                        <Select
                                            value={item.type || ''}
                                            onChange={(e) => handleItemChange('surplus', index, 'type', e.target.value as any)}
                                        >
                                    ...
                                    <TextField
                                        ...
                                        onChange={(e) => handleItemChange('surplus', index, 'quantity', parseInt(e.target.value) || 0)}
                                        sx={{ width: '150px' }}
                                    />
                                    <TextField
                                        label="Ghi chú"
                                        value={item.note || ''}
                                        onChange={(e) => handleItemChange('surplus', index, 'note', e.target.value)}
                                        fullWidth
                                    />
                    ...
                    {modalType === 'need' && (
                        <>
                            <Typography variant="subtitle1" mt={2}>Nhu cầu:</Typography>
                            {formData.needs.map((item, index) => (
                                <Stack key={index} direction="row" spacing={1} alignItems="center" mb={1}>
                                    <FormControl fullWidth margin="normal">
                                        ...
                                        <Select
                                            value={item.type || ''}
                                            onChange={(e) => handleItemChange('needs', index, 'type', e.target.value as any)}
                                        >
                                    ...
                                    <TextField
                                        ...
                                        type="number"
                                        value={item.quantity || 0}
                                        onChange={(e) => handleItemChange('needs', index, 'quantity', parseInt(e.target.value) || 0)}
                                        sx={{ width: '150px' }}
                                    />
                                    <TextField
                                        label="Ghi chú"
                                        value={item.note || ''}
                                        onChange={(e) => handleItemChange('needs', index, 'note', e.target.value)}
                                        fullWidth
                                    />
            ...
            <Dialog open={!!selectedPoint} onClose={handleCloseDetail} maxWidth="sm" fullWidth>
                ...
                <DialogContent>
                    {selectedPoint && (
                        <Paper elevation={3} sx={{ p: 2 }}>
                            ...
                            <List dense>
                                <ListItem>
                                    <ListItemText primary="Mô tả" secondary={selectedPoint.description || 'Không có'} />
                                </ListItem>
                                <ListItem>
                                    <ListItemText primary="Liên hệ" secondary={selectedPoint.contact || 'Không có'} />
                                </ListItem>
                                <ListItem>
                                    <ListItemText primary="Tạo lúc" secondary={new Date(selectedPoint.createdAt).toLocaleString()} />
                                </ListItem>
                            </List>

                            {selectedPoint.type === 'supply' && selectedPoint.surplus && selectedPoint.surplus.length > 0 && (
                                <>
                                    <Divider sx={{ my: 2 }} />
                                    <Typography variant="subtitle1" gutterBottom>Dư thừa:</Typography>
                                    <List dense>
                                        {selectedPoint.surplus.map((s, i) => (
                                            <ListItem key={i}>
                                                <ListItemText primary={s.type} secondary={`${s.quantity} - ${s.note || 'Không có ghi chú'}`} />
                                            </ListItem>
                                        ))}
                                    </List>
                                </>
                            ...
                            {selectedPoint.type === 'need' && selectedPoint.needs && selectedPoint.needs.length > 0 && (
                                <>
                                    <Divider sx={{ my: 2 }} />
                                    <Typography variant="subtitle1" gutterBottom>Nhu cầu:</Typography>
                                    <List dense>
                                        {selectedPoint.needs.map((n, i) => (
                                            <ListItem key={i}>
                                                <ListItemText primary={n.type} secondary={`${n.quantity} - ${n.note || 'Không có ghi chú'}`} />
                                            </ListItem>
                                        ))}
                                    </List>
                                </>
